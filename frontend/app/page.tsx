...
export default function Home() {
  ...
  return <main style={{ maxWidth: 1100, margin: '0 auto', padding: 24 }}><h1>Forge Agent v17 Real Integrations</h1><p>GitHub and Vercel integration-focused build.</p><div style={card}><input value={appName} onChange={e=>setAppName(e.target.value)} style={field}/><textarea value={prompt} onChange={e=>setPrompt(e.target.value)} rows={6} style={field}/><button onClick={submit} style={button}>Queue job</button></div>{result && <pre style={pre}>{JSON.stringify(result,null,2)}</pre>}<div style={{display:'grid',gap:12,marginTop:16}}>{jobs.map(j=><div key={j.id} style={card}><div><strong>{j.app_name}</strong></div><div>Status: {j.status}</div><div>PR: {j.pr_url || '—'}</div><div>Deploy: {j.deployment_url || '—'}</div><div>Deploy state: {j.deployment_state || '—'}</div></div>)}</div></main>
}
