...
logger = logging.getLogger("forge_agent.github")
...
async def get_default_branch_sha(owner, repo):
    """Get default branch name and HEAD SHA. Handles empty repos gracefully."""
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        ref_resp = await client.get(
            f'{GITHUB_API}/repos/{owner}/{repo}/git/ref/heads/{branch}', headers=headers()
        )
        if ref_resp.status_code != 200:
            logger.warning("Could not get ref for %s/%s branch %s: %s", owner, repo, branch, ref_resp.status_code)
            return branch, None
...
async def create_branch(repo, branch_name):
    """Create a feature branch. If the repo is empty (no SHA), skip branch creation."""
    owner = os.getenv('GITHUB_OWNER', '')
    base_branch, sha = await get_default_branch_sha(owner, repo)

    if not sha:
        logger.info("Repo %s/%s appears empty — skipping branch creation, using %s", owner, repo, base_branch)
        return {'status_code': 200, 'base_branch': base_branch, 'data': {'skipped': True, 'reason': 'empty repo'}}

    async with httpx.AsyncClient(timeout=30) as client:
        r = await client.post(
            f'{GITHUB_API}/repos/{owner}/{repo}/git/refs', headers=headers(),
...
async def put_file(repo, branch_name, path, content, message):
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        r = await client.put(
            ...
            json={'message': message, 'content': encoded, 'branch': branch_name},
        )
        return {'status_code': r.status_code, 'data': r.json()}
...
async def create_pr(repo, branch_name, base_branch, title, body):
    """Create a pull request. Returns gracefully if branch doesn't differ from base."""
    owner = os.getenv('GITHUB_OWNER', '')
    async with httpx.AsyncClient(timeout=30) as client:
        r = await client.post(
            f'{GITHUB_API}/repos/{owner}/{repo}/pulls', headers=headers(),
...
async def list_reviews(repo, pull_number):
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        r = await client.get(
            f'{GITHUB_API}/repos/{owner}/{repo}/pulls/{pull_number}/reviews', headers=headers()
        )
        return {'status_code': r.status_code, 'data': r.json()}
